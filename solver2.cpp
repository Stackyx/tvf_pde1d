...
solver_edp2::solver_edp2(model pde_model, mesh grille, bound2 boundary, payoff m, double theta)
	: s_pde_model(pde_model), s_bound(boundary), s_mesh(grille), s_f(m), s_theta(theta)
{
}

void solver_edp2::solve_pde(const bool& vega_bool)
{	
	...
	s_bound.get_boundaries2(s_pde_model.get_r(s_mesh.get_nt()-1), s_pde_model.get_r(s_mesh.get_nt()-1), sigma[0], sigma[s_mesh.get_nt()-1], (s_mesh.get_nt()-1)*dt, dt, (s_mesh.get_nt()-1), solution); 
	...
	for(int i = s_mesh.get_nt() - 1; i > 0; --i)
	{
		...
		s_bound.get_boundaries2(r, s_pde_model.get_r(i-1), sigma[0], sigma[s_mesh.get_nt()-1], (s_mesh.get_nt()-1)*dt, dt, (i-1), solution);
...
void solver_edp2::pde_matrix(std::vector<std::vector<double>>& mat, std::vector<std::vector<double>>& mat_inv, const std::vector<double>& sigma, const std::vector<double>& sigma_plus, double r, double theta, double dt, double dx, int nx, int i)
{	
	mat[1][0] = 1;
	mat[1][nx-1] = 1;
...
void solver_edp2::product_inverse(std::vector<double>& x, std::vector<std::vector<double>> trig_mat, std::vector<double> d)
{
	double w;
...
void solver_edp2::trig_matmul(std::vector<double>& res, std::vector<std::vector<double>> trig_mat, std::vector<double> x)
{	
